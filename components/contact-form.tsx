...
export function ContactForm() {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setIsSubmitting(false)
      showToast("Fehler beim Senden. Bitte versuchen Sie es später erneut.", 'error')
    }
  }
  ...
  return (
    <>
      {toast && (
        <div className={`fixed top-4 right-4 p-4 rounded-md ${toast.type === 'success' ? 'bg-green-500' : 'bg-red-500'} text-white`}>
          {toast.message}
        </div>
      )}
